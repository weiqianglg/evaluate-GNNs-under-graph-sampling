...
class ForestFireSampler(Sampler):
    ...
    def __init__(self, number_of_nodes: int=100, p: float=0.4, seed: int=42, max_visited_nodes_backlog: int=100,
                 restart_hop_size: int = 10, subgraph: bool=True):
        ...
        self._sampled_edges = []
        self.subgraph_induction = subgraph
    ...
    def _start_a_fire(self, graph, start_node):
        """
        ...
        while len(self._sampled_nodes) < self.number_of_nodes:
            ...
            for neighbor in burned_neighbors:
                ...
                self._sampled_edges.append((top_node, neighbor))
    ...
    def sample(self, graph: Union[NXGraph, NKGraph], start_node: int=None) -> Union[NXGraph, NKGraph]:
        ...
        while len(self._sampled_nodes) < self.number_of_nodes:
            self._start_a_fire(graph, start_node)
            start_node = None
        if self.subgraph_induction:
